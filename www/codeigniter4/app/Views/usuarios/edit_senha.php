...
if (isset($_SESSION['login'])) {
    ...
    // Permite qualquer usuário logado trocar a própria senha (nível 0, 1, 2, 3...)
    ?>
    <?= $this->extend((isset($login->usuarios_nivel) && $login->usuarios_nivel == 1) ? 'Templates_admin' : 'Templates_user') ?>
    <?= $this->section('content') ?>

    <div class="container">
        <?php if (isset($msg)) { echo $msg; } ?>

        <h2 class="border-bottom border-2 border-primary mt-3 mb-4">Alterar Senha</h2>
        ...
        <form action="<?= base_url('usuarios/salvar_senha'); ?>" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="password" class="form-control" name="usuarios_nova_senha"
                    value="<?= isset($usuarios->usuarios_nova_senha) ? $usuarios->usuarios_nova_senha : '' ?>" id="usuarios_nova_senha">
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="password" class="form-control" name="usuarios_confirmar_senha"
                    value="<?= isset($usuarios->usuarios_confirmar_senha) ? $usuarios->usuarios_confirmar_senha : '' ?>" id="usuarios_confirmar_senha">
            </div>
